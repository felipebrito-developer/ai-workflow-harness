...
describe("CircuitBreaker Engine", () => {
  ...
  it("should record attempt failures and trip after max limit", async () => {
    ...
    const res1 = await CircuitBreaker.recordFailure(taskId, "bun test", "Syntax error line 5", 3);
    expect(res1.tripped).toBe(false);
    expect(res1.currentAttempts).toBe(1);

    const res2 = await CircuitBreaker.recordFailure(taskId, "bun test", "Syntax error line 5", 3);
    expect(res2.tripped).toBe(false);
    expect(res2.currentAttempts).toBe(2);

    const res3 = await CircuitBreaker.recordFailure(taskId, "bun test", "Syntax error line 5", 3);
