...
export async function runAgentCreate(initialName?: string): Promise<void> {
    ...
    const questions: Parameters<typeof enquirer.prompt>[0] = [
        ...
        },
        {
            ...
            initial: "Performs security scans and analyzes dependency vulnerabilities.",
        },
        {
            type: "select",
            name: "mode",
        ...
        },
        {
            ...
            choices: [
                { name: "read_only", message: "Read-Only / Auditor (edit: deny, bash: ask, task: deny)" },
                { name: "runner", message: "Test / Script Runner (edit: deny, bash: allow, task: deny)" },
                { name: "worker", message: "Autonomous Worker (edit: allow, bash: allow, task: deny)" },
            ],
        },
        {
            type: "input",
            ...
            initial: "You are a specialized agent. Enforce project standards and report concise summaries.",
        },
    ];
    ...
    if (answers.permissionPreset === "read_only") {
        permissions = { edit: "deny", bash: "ask", task: { "*": "deny" }, externalDirectory: "deny" };
        ...
        permissions = { edit: "allow", bash: "allow", task: { "*": "deny" }, externalDirectory: "deny" };
    }
    ...
    console.log(chalk.dim(`- Canonical Spec: .harness/agents/${validatedAgent.name}.json`));
    console.log(chalk.dim(`- Synchronized Adapter Files:\n  ${compiledFiles.map((f) => `• ${f}`).join("\n  ")}`));
