...
export async function runMcpAdd(initialName?: string): Promise<void> {
	...
	const questions: Parameters<typeof enquirer.prompt>[0] = [
		{
			...
			choices: [
				{
					name: "filesystem",
					message: "Filesystem (@modelcontextprotocol/server-filesystem)",
				},
				{
					name: "github",
					message: "GitHub (@modelcontextprotocol/server-github)",
				},
				{
					name: "postgres",
					message: "PostgreSQL (@modelcontextprotocol/server-postgres)",
				},
				{
					name: "fetch",
					message: "Fetch Web (@modelcontextprotocol/server-fetch)",
				},
				{
					name: "linear",
					message: "Linear (@modelcontextprotocol/server-linear)",
				},
		...
		},
		{
			...
			validate: (val: string) =>
				/^[a-z0-9_-]+$/.test(val) ||
				"Must be valid identifier (a-z, 0-9, -, _)",
		},
		{
			type: "input",
			name: "commandString",
	...
	const type: "local" | "remote" =
		answers.preset === "custom_remote" ? "remote" : "local";
	let command: string[] = [];
	...
	console.log(
		chalk.dim(`- Canonical Spec: .harness/mcp/${validatedServer.name}.json`),
	);
	console.log(
		chalk.dim(
