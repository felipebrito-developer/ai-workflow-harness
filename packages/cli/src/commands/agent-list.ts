...
export async function runAgentList(): Promise<void> {
    ...
    const header =
        "  " +
        chalk.bold("AGENT".padEnd(colName)) +
        chalk.bold("MODE".padEnd(colMode)) +
        chalk.bold("PROVIDER".padEnd(colProvider)) +
        chalk.bold("MODEL".padEnd(colModel)) +
        chalk.bold("CACHING".padEnd(colCaching));

    console.log(header);
    console.log(chalk.dim("  " + "─".repeat(colName + colMode + colProvider + colModel + colCaching)));
    ...
    for (const agent of agents) {
        ...
        const modeColor = agent.mode === "primary" ? chalk.magenta(agent.mode) : chalk.dim(agent.mode);
        const cachingColor =
        ...
        const renderedName = agent.source === "default" ? chalk.blue(namePadded) : chalk.green(namePadded);
