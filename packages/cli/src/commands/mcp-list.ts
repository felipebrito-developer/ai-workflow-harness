...
export async function runMcpList(): Promise<void> {
	...
	if (mcpList.length === 0) {
		...
		console.log(
			chalk.dim("  Run `harness mcp:add` to configure an MCP server.\n"),
		);
		return;
	}
	...
	const header = `  ${chalk.bold("NAME".padEnd(colName))}${chalk.bold("TYPE".padEnd(colType))}${chalk.bold("COMMAND / URL".padEnd(colTarget))}${chalk.bold("ENV KEYS".padEnd(colEnv))}`;

	console.log(header);
	console.log(
		chalk.dim(`  ${"─".repeat(colName + colType + colTarget + colEnv)}`),
	);
	...
	for (const item of mcpList) {
		...
		const typeColor =
			item.type === "remote" ? chalk.magenta(item.type) : chalk.dim(item.type);
