export class TemplateScaffolder {
	...
	public static getSpecQueryMcpServer(): string {
		return [
			...
			'    if (!feat) return { content: [{ type: "text", text: `Error: Feature "${slug}" not found in database.` }] };',
			...
			"    checkpoint();",
			'    return { content: [{ type: "text", text: `✔ Successfully wrote spec for ${slug}/${category}/${topicSlug} (WAL checkpointed).` }] };',
			"  }",
			...
			"    checkpoint();",
			'    return { content: [{ type: "text", text: `✔ Updated feature "${slug}" status to ${status}.` }] };',
			"  }",
			'  if (name === "create_task") {',
			...
			"    checkpoint();",
			'    return { content: [{ type: "text", text: `✔ Task "${taskId}" created/updated in database.` }] };',
			"  }",
			...
			"    checkpoint();",
			'    return { content: [{ type: "text", text: `✔ Updated task "${taskId}" status to ${status}.` }] };',
			"  }",
