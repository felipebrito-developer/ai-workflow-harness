...
export class SecurityScanner {
	private static SECRET_PATTERNS = [
		{
			...
			pattern:
				/(api_key|apikey|secret_key|auth_token)\s*=\s*['"][A-Za-z0-9_\-]{12,}['"]/i,
		},
	...
	public static async scanDiffSecrets(
		diffText: string,
		...
		for (const line of lines) {
			if (line.startsWith("+++ b/")) {
				...
				for (const rule of SecurityScanner.SECRET_PATTERNS) {
	...
	public static async scanRepoSecrets(cwd: string): Promise<SecurityIssue[]> {
		...
		try {
			...
			// Check if gitignored
			try {
				const { stdout } = await execa("git", ["check-ignore", ".env"], {
					cwd,
				});
				if (!stdout.includes(".env")) {
					issues.push({
						...
						file: ".env",
						summary:
							".env file exists in repository root and is NOT gitignored",
						severity: "CRITICAL",
					});
				}
			} catch {
	...
	public static async runDependencyAudit(
		cwd: string,
	): Promise<SecurityIssue[]> {
		const issues: SecurityIssue[] = [];
		...
		try {
			...
			try {
				const { stdout, stderr } = await execa("bun", ["audit"], { cwd });
				if (
					stdout.includes("vulnerabilities found") ||
					stderr.includes("vulnerabilities found")
				) {
					issues.push({
						type: "DEPENDENCY_VULN",
						summary:
							"Package vulnerabilities detected via bun audit. Run 'bun audit' to inspect.",
						severity: "HIGH",
					});
				}
			} catch (err: any) {
	...
	public static formatSecurityCard(issues: SecurityIssue[]): string {
		...
		for (const issue of issues) {
			lines.push(
				chalk.red(
					`│ [${issue.severity}] ${issue.type.padEnd(16)} │ ${issue.summary.slice(0, 30).padEnd(30)}│`,
				),
			);
			if (issue.file) {
				lines.push(
					chalk.dim(`│   Target File: ${issue.file.slice(0, 40).padEnd(40)}│`),
				);
			}
		}
