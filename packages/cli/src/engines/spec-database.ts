...
export class SpecDatabase {
  ...
  public getTopicChunk(topicSlug: string, level: "summary" | "detail"): string | null {
    const query = this.db.prepare(`
      SELECT c.content FROM spec_chunks c
      JOIN spec_topics t ON c.topic_id = t.id
      WHERE t.slug = $topicSlug AND c.level = $level
    `);
    const res = query.get({ $topicSlug: topicSlug, $level: level }) as { content: string } | null;
  ...
  public async exportToMarkdown(harnessDir: string): Promise<void> {
    ...
    // Export App Summary
    const features = this.db.prepare("SELECT * FROM features").all() as SpecFeature[];
    ...
    await fs.writeFile(path.join(specDir, "app-summary.md"), appSummaryLines.join("\n"), "utf-8");
    ...
    for (const feat of features) {
      ...
      `).all({ $featId: feat.id }) as { category: string; title: string; level: string; content: string }[];
