...
export class GitManager {
	...
	public static async ensureTaskBranch(
		taskId: string,
		titleSlug: string,
	): Promise<string> {
	...
	public static async getChangedFiles(): Promise<string[]> {
		...
		const { stdout: untracked } = await execa("git", [
			"ls-files",
			"--others",
			"--exclude-standard",
		]);
		...
		for (const changed of changedFiles) {
			if (
				SYSTEM_PREFIXES.some(
					(prefix) => changed.startsWith(prefix) || changed === prefix,
				)
			) {
				continue;
			}
			...
			const isAllowed = allowedFiles.some((allowed) => {
				...
				if (
					allowed.endsWith("/**") &&
					changed.startsWith(allowed.replace("/**", ""))
				)
					return true;
				return false;
			});
	...
	public static async rollbackAllowedFiles(
		allowedFiles: string[],
	): Promise<void> {
