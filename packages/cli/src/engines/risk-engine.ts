...
export class RiskEngine {
	public static evaluateTaskRisk(
		...
		if (files.length > 5) {
			score += 4;
			reasons.push(
				`Touches ${files.length} files (exceeds recommended 2-file limit)`,
			);
		} else if (files.length > 2) {
			score += 2;
			reasons.push(
				`Touches ${files.length} files (exceeds 2-file ideal scope)`,
			);
		}
	...
	public static formatRiskCard(assessment: TaskRiskAssessment): string {
		...
		if (
			assessment.recommendedSlices &&
			assessment.recommendedSlices.length > 0
		) {
			lines.push(
				color("├────────────────────────────────────────────────────────┤"),
			);
			lines.push(
				color("│ Recommended Sub-Slices:                                │"),
			);
